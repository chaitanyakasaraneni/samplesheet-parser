...
Covers:
- hamming_distance helper directly
...
def _distance_warnings(result: ValidationResult) -> list[Any]:
...
class TestHammingDistance:
    def test_identical_sequences(self):
        assert hamming_distance("ATTACTCG", "ATTACTCG") == 0

    def test_one_mismatch(self):
        assert hamming_distance("ATTACTCG", "ATTACTCA") == 1

    def test_two_mismatches(self):
        assert hamming_distance("ATTACTCG", "ATTACTAA") == 2
    ...
    def test_symmetric(self):
        assert hamming_distance("ATTACTCG", "ATTACTCA") == hamming_distance("ATTACTCA", "ATTACTCG")

    def test_minhamming_distance_constant(self):
...
""",
        ...
        assert hamming_distance("ATTACTCG", "GTTAAACG") == 3
        result = _validate(sheet)
        assert _distance_warnings(result) == []
...
""",
        ...
        combined_dist = hamming_distance(
            "ATTACTCG" + "TATAGCCT",
            "ATTACTCA" + "GCTAGCTA",
        )
...
""",
        ...
        SampleSheetValidator()._check_index_distances(samples, result, min_distance=5)
        w = _distance_warnings(result)[0]
